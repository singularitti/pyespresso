...
if not calculate_tensor:
	...
	for i in press:
		...
		if os.path.exists(dir_name):
			print('Scf folder for P = %d exists. Skiping qe input generation and creating Cij files.' % i)
			signal_scf = False
		else:
			print('Scf folder for P = %d does not exist. Creating input files for pw and Cij.' % i)
		...
		if os.path.exists(dir_name + '/' + cij_dir_name):
			...
			functions.create_cij_files(vectors, alat, dir_name, cij_dir_name, i, atom_pos, dist, nstrain, cclass)
	...
	if signal_scf:
		...
		functions.create_job(job_lines, press, div_scf, 'job-scf.sh', 'scf_', 'pw', modul)
	...
	functions.create_job_cij(job_lines, press, div, 'job-cij.sh', 'scf_', cclass, nstrain, dist, modul)

	if submit_job:
		signal_job_cij = True	
		job_sub_cij = subprocess.Popen(['sbatch', 'job-cij.sh'], stdout=subprocess.PIPE)
