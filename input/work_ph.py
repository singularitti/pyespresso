...
with open('ph_input_data', 'r') as ph_data:
    lines_ph = ph_data.readlines()
    for i in range(0, len(lines_ph)):
        if lines_ph[i].startswith('Submit'):
            ph_will_be_submitted = lines_ph[i].split(':')[1].rstrip().lstrip()

if ph_will_be_submitted == 'y' or ph_will_be_submitted == 'Y':
    submit_ph = True
else:
    submit_ph = False
...
if submit_ph:
    ...
    print('Job will not be submitted.')
