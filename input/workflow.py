...
dirs_cg = os.listdir(working_dir)
...
#	This part monitors the job execution. 
	while signal_job_cg:
		job_status = subprocess.Popen(['squeue', '-j', str(job_id)], stdout=subprocess.PIPE) # request SLURM queue information
...
for i in press:
	if erro in os.listdir('cg_' + str(i)):
		print('Something went wrong, CRASH files found. Check your cg outputs.') # If CRASH files exist, something went wrong. Stop the workflow
		quit()
	else:
...
if Number_error_files > int(npress/2):
	print('More than a half of pressures were not calculated. Exiting') # If more that a half of the calculations are not found, stop the workflow
	quit()

press = np.delete(press,index_list) # deletes the pressures that weren't found

if len(press) != len(Paux) or len(Paux) != len(Vaux): # If the number of volumes are not equal to the number of pressures, something is wrong, stop the workflow
